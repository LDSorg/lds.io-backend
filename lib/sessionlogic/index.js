      ...
      function finishLoginHelper(err) {
        ...
        function finish() {
          ...
          if (!loginResults.successUrl) {
            console.log('[loginResults] strange login results');
            console.log(loginResults);
            nextOrCallback(null, req.user);
          } else {
            // TODO pass back token
            // req.url = loginResults.successUrl || req.url;
            res.redirect(loginResults.successUrl + '&code=success');
          }
        }
      ...
      if (!config.trustedOrigins.some(function (origin) {
        ...
        res.error({
          message: "Invalid Origin '"
            + encodeURI(req.headers.origin || req.headers.referer) 
            + "' for " 
            + encodeURI(config.apiPrefix) 
            + " access"
        , code: 401
