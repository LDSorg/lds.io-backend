...
module.exports.init = function (passport, config, Auth, OauthClients, AccessTokens, Logins) {
  ...
  function loginWrapper(req, res, nextOrCallback, loginResults) {
    ...
    getLoginFromResults(loginResults).then(function () {
      ...
      //logins$ = reqUser.logins$ || reqUser.$logins || reqUser.logins || [loginResults.user.$login];
      //logins$ = [loginResults.user.$login];

      newReqUser = {
        $token: loginResults.user.$token || reqUser.$token
      , $login: loginResults.user.$login
      , accounts$: accounts$
      /*
      ...
      */
      ...
      function finishLoginHelper(err) {
        ...
        function finish() {
          ...
          // this only triggers on grantType=password and grantType=delegated
      ...
      */
      ...
      req.logIn(newReqUser, { session: false }, finishLoginHelper);
    });
  }
  ...
  function testForAndLoginWithBearerToken(req, res, next) {
    ...
    passport.authenticate('bearer.st', function (err, user, info) {
      ...
      , user: user
      , info: info
        // Note: not sure if this is even necessary here
      , session: false
      };
