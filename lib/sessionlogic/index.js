...
module.exports.init = function (passport, config, Auth, OauthClients, AccessTokens, Logins) {
  ...
  function testForAndLoginWithBearerToken(req, res, next) {
    ...
    passport.authenticate('bearer.st', function (err, user, info) {
      ...
      , user: user
      , info: info
      // TODO don't require sessions
      // Note 1
      // the current problem is that if you have a cookie session,
      // the token will be overwritten by the session middleware
      // and you won't have the credentials of the token.
      // Note 2
      // If you need to have multiple logins (say to link an account
      // to a login) there isn't way to do that yet, unless both logins
      // are in the session.
      // A fix would be to send tokens for logins rather than IDs for logins
      //, session: false
      };
