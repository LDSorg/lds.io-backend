...
['profile', 'photo', 'ward', 'wardPhotos', 'stake'].forEach(wrapApiWithRetry);
...
module.exports.api.wardPhotos = function (ldsMobileApi, jar, wardId/*, opts*/) {
  ...
  , jar: jar
    ...
    var iids = utils.getIndividualIds(ward);
    var hids = utils.getHomeIds(ward);
    var promises = [];
    var results = { members: [], families: [] };

    var individualPhotoUrls = utils.buildPhotoUrls(ldsMobileApi['photo-url'], iids, 'individual', 50, []);
    var householdPhotoUrls = utils.buildPhotoUrls(ldsMobileApi['photo-url'], iids, 'household', 50, []);
...
module.exports.api.ward = function (ldsMobileApi, jar, wardName, wardId/*, opts*/) {
  ...
  , callApi({
    ...
    sorted.members.forEach(function (m) {
      m.callings = [];
      utils.getCallings({/*id, ward|stake*/}, ward.ward.callings, m.id, [], m.callings);
...
module.exports.api.profile = function (ldsMobileApi, jar/*, opts*/) {
  ...
  , url: ldsMobileApi['current-user-detail']
    ...
    , stakes: results.user.units.map(function (stake) {
        var myStake = {
          id: stake.unitNo
        , name: stake.unitName
