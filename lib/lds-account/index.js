...
module.exports.api.profile = function (ldsMobileApi, jar, opts) {
  ...
  return module.exports.api.user(ldsMobileApi, jar).then(function (body) {
    ...
    */
    ...
    var utils = require('./basic');
    var me = results.ward.members.filter(function (m) {
      if (m.id === individualId) {
        return true;
      }
    })[0];
    var profile = utils.getProfile(results.user, results.ward);
    ...
    , home: results.ward.homes.filter(function (h) {
        if (h.id === me.homeId) {
          return true;
        }
      })[0]
    ...
    //, meetinghouse: results.meetinghouse
    //, stakecenter: results.stakecenter
    ...
    , wardsWithCalling: profile.wardsWithCalling
    , stakesWithCalling: profile.stakesWithCalling
    };
  });
};
