    ...
    unauthorized.forEach(function (login) {
      ...
      promise = promise.then(function () {
        ...
        return Auth.AccessTokens.login(login.token).then(function ($token) {
          ...
          var $login = $token.$login; //$token.related('login');
...
//attachLogins
  ...
  Accounts.publish = function ($client, $account/*, opts*/) {
    ...
    */
    ...
    // TODO appScopedId
    pub.id = json.uuid;
    pub.appScopedId = cipher(json.uuid, $client.get('secret'));
...
module.exports.createRouter = function (app, config, DB, Auth) {
  ...
  Accounts.restful.create = function (req, res) {
    ...
    , requestedLogins).then(function ($account) {
      ...
      //$account.toJSON()
      res.send(Accounts.publish($client, $account));
    }, function (err) {
      res.send({
        error: { 
  ...
  function requireAccount(req, res, next) {
    ...
    if (!req.oauth3.$login) {
      res.error({
        code: "E_SANITY_FAIL"
      , message: "no login, invalid login token, or expired login token"
      });
      return;
    }

    if (!Array.isArray(req.oauth3.accounts$)) {
      res.error({
        code: "E_SANITY_FAIL"
      , message: "[Developer Error] no accounts array. Bad token perhaps?"
      });
    }

    if (!req.oauth3.accounts$.some(function ($acc) {
      if ($acc.id === decipher(req.params.accountId, $client.get('secret'))) {
        req.$account = $acc;
        return true;
      }
    })) {
      ...
      , message: "The specified account is not associated with the login token."
      });
      return;
    }
