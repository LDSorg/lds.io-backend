...
module.exports.create = function (passport, config, DB, AccessTokens, AppLogin, Logins) {
  ...
  // code.
  server.exchange(oauth2orize.exchange.code({ userProperty: 'oauth3' }, function ($apikey, code, redirectURI, done) {
  ...
  server.exchange(oauth2orize.exchange.password({ userProperty: 'oauth3' }, function ($apikey, username, passphrase, scopeArr, done) {
  ...
  server.exchange(oauth2orize.exchange.clientCredentials({ userProperty: 'oauth3' }, function ($apikey, scope, done) {
  ...
  function route(rest) {
    ...
    , function restfulGetScopeDelta(req, res) {
        ...
        if (!oauth2Info) {
          res.error({
            message: "Bad transaction token. Please close this login window, refresh the page you were coming from, and try again."
    ...
    function parseDecision(req, done) {
      ...
      var selectedAccountId = decipher(
        req.body.selectedAccountId || req.body.selected_account_id
