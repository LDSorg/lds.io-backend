...
module.exports.create = function (passport, config, DB, AccessTokens, AppLogin, Logins) {
  var oauthPrefix = config.oauthPrefix;
  var server;
      // in-memory only
  var TxTok;
  ...
  server.exchange(oauth2orize.exchange.password(function ($apikey, username, passphrase, scopeArr, done) {
  ...
  function route(rest) {
    ...
    //)
    rest.get(
      config.oauthPrefix + '/scope/:token'
    , function restfulGetScopeDelta(req, res) {
        ...
        if (!oauth2Info) {
          res.error({
            message: "Bad state token. Please close this login window, refresh the page you were coming from, and try again."
          });
          return;
        }
