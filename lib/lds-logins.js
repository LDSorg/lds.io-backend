...
module.exports.createController = function (config, DB, ContactNodes) {
  ...
  Logins.createLoginOnly = function (secret, recoverynodes, opts) {
    ...
    return $login
      ...
      , { method: 'insert'
        ...
        console.error("[DB Error] couldn't save login at create");
        console.error(err);
  ...
  */
  ...
  Logins.loginTypes.ldsaccount = function (type, node, secret/*, tokens*//*, opts*/) {
    var ldsAccount = require('./lds-account');
    return ldsAccount.createSession(node, secret).then(function (session) {
      secret = '[PROTECTED]';
      ...
      return Logins.getByNode('username', node).then(function ($login) {
        ...
        $login.set('lastSessionAt', (new Date()).toISOString());
        $login.set('profile', {});
      ...
      var promise = $login.load(['accounts']).then(function () {
        if ($login.related('accounts').length > 1) {
          // TODO automated reporting
          return PromiseA.reject("LDS Login is corrupt: attached to multiple accounts."
            + " Please contact support@ldsconnect.org");
        }
        ...
        // TODO prefetch and sync LDS Account with $account
        return $login;
      });
...
module.exports.createView = function (config, DB, manualLogin, ContactNodes) {
  ...
  Logins.restful.getResetCode = function (req, res) {
    ...
    var conf = req.oauth3.$client && req.oauth3.$client.get('config') || {};
  ...
  Logins.restful.create = function (req, res) {
    ...
    , { recoverynodes: recoverynodes, multiauth: multiauth }
      ...
      // TODO this should send a grantType=password token
  ...
  Logins.restful.login = function (req, res) {
    ...
    var promise = Logins.login(type, node, secret, tokens).then(function ($login) {
      ...
      res.error({ code: 401, message: "invalid login credentials" });
    }).error(function (err) {
      if (!(err || err.message)) {
        err = err || {};
  ...
  Logins.restful.update = function (req, res) {
    ...
    var accountsMap = req.accountsMap;
  ...
  Logins.restful.validateReset = function (req, res) {
    ...
    var promise = Logins.validateReset(auth, authcode).then(function () {
      ...
      return manualLoginWrapped(auth.uid, auth.secret).then(function () {
        // TODO this should issue a grantType=password token
        res.send({ success: true });
      });
    });
...
module.exports.create = module.exports.createRouter = function (app, config, DB, manualLogin, ContactNodes) {
  ...
  function requireLogin(req, res, next) {
    ...
    req.oauth3.logins$.forEach(function ($login) {
      if (hashid === $login.id) {
        req.$login = $login;
      }
    });
    ...
    req.accountsMap = {};
    req.oauth3.accounts$.forEach(function ($acc) {
