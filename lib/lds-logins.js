...
module.exports.createController = function (config, DB, ContactNodes) {
  ...
  */
  ...
  Logins.loginTypes.ldsaccount = function (type, node, secret/*, tokens*//*, opts*/) {
    ...
    return require('./lds-account').login({
      username: node
    , password: secret
      ...
      return Logins.getByNode('username', node).then(function ($login) {
        ...
        Object.keys(session).forEach(function (key) {
          ...
          if (-1 !== ['homes', 'individuals', 'ward', 'stakes'].indexOf(key)) {
            return;
          }
        ...
        profile.ward = undefined;
        profile.callingWards = undefined;
        profile.callingStakes = undefined;
        /*
        ...
        return $login.save().then(function ($login) {
          ...
          var promises = $login.load('accounts').then(function () {
            ...
            var promises2 = $login.related('accounts').map(function ($account) {
              ...
              if (session.me.emails[0]) {
                email = (session.me.emails[0].value || '').toLowerCase().trim();
                if (pub.email !== email) {
              ...
              if (session.me.phones[0]) {
                phone = (session.me.phones[0].value || '').toLowerCase().trim();
                if (pub.phone !== phone) {
