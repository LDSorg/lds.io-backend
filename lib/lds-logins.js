...
module.exports.createController = function (config, DB, ContactNodes) {
  ...
  */
  ...
  Logins.markAsChecked = function ($login) {
    ...
    profile.checkedAt = parseInt(Date.now() / 1000, 10);
  ...
  Logins.loginTypes.ldsaccount = function (type, node, secret/*, tokens*//*, opts*/) {
    ...
    return PromiseA.resolve().then(function () {
      return require('./lds-account').createSession(node, secret);
    }).then(function (session) {
      secret = '[PROTECTED]';
      session.lastSessionAt = (new Date()).toISOString();
      ...
      , lastSessionAt: session.lastSessionAt
        ...
        // TODO in account creation
        // { individualId, name, givennames, surnames, phones, emails, home, photos, callings, stakes
        // , homeWardId, homeWardName, homeStakeId, homeStakeName, callings, wardsWithCalling, stakesWithCalling }
        Object.keys(profile.result).forEach(function (key) {
          session[key] = profile.result[key];
        });
        return session;
      });
    }).then(function (session) {
      ...
      , jar: session.jar
      , lastSessionAt: session.lastSessionAt
      }, session.homeWardName, session.homeWardId).then(function (ward) {
        // TODO remove
        session.leaders = ward.result.leaders;
      ...
      return Logins.getByNode('username', node).then(function ($login) {
        ...
        console.error('needs to createLdsAccount (login prolly don\'t exist)');
        console.error(err);
        // catchable errors are due to the account not existing
        return Logins.createLdsAccount(type, node, {});
      }).then(function ($login) {
        ...
        profile.ts = Date.now();
        if (!session.token) {
          return PromiseA.reject(new Error("SANITY FAIL: No token"));
        }
        if (!session.token) {
          return PromiseA.reject(new Error("SANITY FAIL: No jar"));
        }

        $login.set('token', session.token);
        $login.set('jar', session.jar);
        // TODO touch this (only) when near expiry to keep using the same jar
        $login.set('lastSessionAt', Date.now());
        $login.set('profile', profile);

        // NOTE: On creation this is the second save, but the performance is a small price to pay for consolidated code.
        return $login.save().then(function ($login) {
          // sync with account
          var promises = $login.load('accounts').then(function () {
            if ($login.related('accounts').length > 1) {
              // TODO automated reporting
              return PromiseA.reject("LDS Login is corrupt: attached to multiple accounts. Please contact support@ldsconnect.org");
            }

            if (0 === $login.related('accounts').length) {
              // TODO Create Account instead
              return $login;
            }
            ...
            // Sync between LDS.org and LDS Connect on each LDS login
            // TODO sync on facebook login as well
            var promises2 = $login.related('accounts').map(function ($account) {
              var pub;
              var email;
              var phone;

              $account.set('token', $login.get('token'));
              $account.set('jar', $login.get('jar'));
              pub = $account.get('public');

              if (session.emails[0]) {
                email = (session.emails[0].value || '').toLowerCase().trim();
                if (pub.email !== email) {
              ...
              if (session.phones[0]) {
                phone = (session.phones[0].value || '').toLowerCase().trim();
                if (pub.phone !== phone) {
