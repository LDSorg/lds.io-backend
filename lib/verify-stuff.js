...
module.exports.createController = function (config, DB, ContactNodes) {
  ...
  Verifier.getAndSendClaimCode = function (conf, type, node) {
    ...
    return Verifier.getClaimCode(type, node).catch(function (err) {
      console.error('[Verifier.getClaimCode] cb 1', type, node, err);
      throw err;
    }).then(function ($code) {
      return Verifier.sendAuthCode(conf, messages, $code).catch(function (err) {
        console.error('[Verifier.sendAuthCode] cb 3', err);
        // TODO
        throw err;
      }).then(function ($code) {
        return $code;
...
module.exports.createView = function (config, DB, ContactNodes) {
  ...
  Verifier.restful.getClaimCode = function (req, res) {
    ...
    var promise = Verifier.getAndSendClaimCode(conf, type, node).then(function ($code) {
      return { uuid: $code.id };
    });
  ...
  Verifier.restful.getClaimCodes = function (req, res) {
    var $client = (req.user && req.user.$client || req.$client || req.client);
    //console.log("Verifier.restful.getClaimCodes $client");
    //console.log($client);
    var conf = $client && ($client.get && $client.get('config') || $client.config) || {};
    var promises = [];
    var nodes = req.body;
    ...
    nodes.forEach(function (node) {
      ...
      promises.push(
        Verifier.getAndSendClaimCode(conf, type, node.node)
          .then(function ($code) {
            return { uuid: $code.id };
          })
          .catch(function (err) {
            console.error("Verifier.restful.getClaimCodes");
            console.error(err);
            throw err;
  ...
  Verifier.restful.validateClaimCodes = function (req, res) {
    ...
    codes.forEach(function (code) {
      ...
      promises.push(Verifier.validateClaimCode(type, node, id, _code, { destroyOnceUsed: false }).then(function () {
        return {
          type: type
        , node: node
        , validated: true
