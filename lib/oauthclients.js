...
module.exports.createController = function (config, Db) {
  ...
  OauthClients.createKeys = function (config, $client, opts) {
    ...
    if (opts.test) {
      keypair.key = 'pub_test_' + (opts.key || authutils.random(idByteLen, 'hex'));
      if (!keypair.secret) {
        keypair.secret = 'sec_test_' + (opts.secret || authutils.random(secByteLen, 'hex'));
      }
    } else {
      keypair.key = 'pub_prod_' + authutils.random(idByteLen, 'hex');
      if (!keypair.secret) {
        keypair.secret = 'sec_prod_' + authutils.random(secByteLen, 'hex');
      }
    }
